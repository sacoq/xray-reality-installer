...
class Server(Base):
    ...
    #
    # Maintained as the legacy "is in primary tier" flag — kept in
    # sync with ``pool_tier='primary'`` by the API layer. Existing
    # code paths that filter by ``in_pool`` (the balancer's
    # ``pool_upstreams`` query, telegram-bot key fan-out, etc.) keep
    # working unchanged; the new fallback tier is opt-in via
    # ``pool_tier='fallback'`` and does NOT flip ``in_pool``.
    ...
    # subscription renderer. Values:
    #
    #   ``""``       — not in any tier (default).
    #   ``primary``  — preferred tier. Picked by the client whenever
    #                  reachable. Typical: foreign exit nodes that
    #                  give the user direct internet egress.
    #   ``fallback`` — secondary tier. Picked only when every primary
    #                  server is unreachable from the user. Typical:
    #                  whitelist-front chain nodes that live on RU
    #                  whitelisted IPs and forward traffic to a
    #                  foreign upstream — used when ТСПУ flips on
    #                  in the user's region and the foreign IPs
    #                  stop responding.
    #
    # The subscription emits a hierarchical sing-box ``urltest`` over
    # both tiers with a 30s probe interval (configurable). When the
    # primary tier goes dark for a user, the urltest auto-switches to
    # the fallback group; once primary recovers, the urltest switches
    # back. No admin intervention, no panel-side daemon — sing-box /
    # Hiddify / Karing / Clash all do this natively.
    pool_tier: Mapped[str] = mapped_column(String(16), nullable=False, default="")
    ...
    # Optional comma-separated list of extra Reality serverNames the front
    # accepts. The default ``sni`` above is always allowed; this column
    # adds more so different clients can use different SNIs in their
    # vless:// link without spinning up extra inbounds. Mobile RU
    # operators (Tele2, Yota, …) often blacklist a single popular SNI,
    # so an admin will keep e.g. ``ya.ru`` as the default and add
    # ``mail.ru,vk.com,dzen.ru`` here — when a key with one SNI starts
    # failing on a network, the user's key is moved to a different SNI
    # without rotating the UUID.
    #
    # Reality probe-protection caveat: ``dest`` is still single per
    # inbound, so an active probe with an SNI that doesn't match
    # ``dest``'s cert can detect the mismatch. Acceptable for passive
    # mobile DPI; not safe against motivated active probing.
    extra_snis: Mapped[str] = mapped_column(Text, nullable=False, default="")
...
class Client(Base):
    ...
    # Per-client Reality SNI used when building this client's
    # ``vless://`` link. Must be one of the values in
    # ``server.sni`` ∪ ``server.extra_snis``. NULL = use the server's
    # default (``server.sni``). Lets one whitelist-front node serve
    # different keys with different SNIs without spinning up extra
    # inbounds — useful when a particular SNI gets DPI-flagged on a
    # mobile operator and we want to switch a user to a different
    # SNI without rotating the UUID.
...
def server_all_snis(server: Server) -> list[str]:
    """Return the unique, in-order list of SNIs the server accepts.

    The default ``server.sni`` is always first; ``extra_snis`` (a
    comma-separated string) is appended after, with duplicates and
    blanks dropped. This is the list that ends up in xray's
    ``serverNames`` for the inbound, and that the panel UI surfaces
    as the SNI dropdown when creating / editing a client.
    """
    base = (server.sni or "").strip()
    out: list[str] = []
    seen: set[str] = set()
    if base:
        out.append(base)
        seen.add(base)
    for raw in (server.extra_snis or "").split(","):
        sni = raw.strip()
...
def client_effective_sni(client: Client, server: Server) -> str:
    """The SNI to use in this client's vless:// link.

    Falls back to ``server.sni`` when the client doesn't pin a specific
    one (``client.sni IS NULL`` — the legacy behaviour for clients
    created before the multi-SNI feature shipped). If the client's
    pinned SNI is no longer in the server's allowed list (e.g. an
    admin removed an extra SNI without re-pinning the affected
    clients) we still hand it back — xray will reject the handshake
    and the admin will notice. Better than silently rewriting the
    link.
    """
    pinned = (client.sni or "").strip()
    if pinned:
        return pinned
...
class EnrollmentToken(Base):
    ...
    # off.
    in_pool: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    # Pre-set auto-balance tier — applied to the Server on enrollment.
    # See ``Server.pool_tier`` for the value semantics. The dashboard's
    # «⚡ Авто-балансировка» button sets this to ``primary``; the
    # «🛡 Нода обхода (whitelist)» button sets it to ``fallback`` (so
    # the new front lands in the fallback tier of subscriptions
    # automatically). Empty for everything else.
    pool_tier: Mapped[str] = mapped_column(String(16), nullable=False, default="")
