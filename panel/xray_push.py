...
def push_custom_config(
    server: Server, *, remove_emails: Iterable[str] = ()
) -> None:
    """Reconcile panel-owned users inside one externally-owned inbound.

    Unknown users are preserved.  Structural fields are never generated by
    the panel: it fetches the current config, changes only
    ``settings.clients`` on ``custom_inbound_tag`` and sends the result back to
    the agent.  The agent's config diff then applies the user delta through
    xray's runtime API and persists it without a restart.
    """
...
def push_config(
    ...
    *,
    ...
    """
    if is_custom(server):
        push_custom_config(server, remove_emails=remove_emails)
