...
with SessionLocal() as db:
    ...
    if db.query(Server).count() == 0:
        ...
        pushed_config = build_config(
            port=s.port, server_names=[s.sni], dest=s.dest,
...
main() {
    ...
    if [[ "$NODE_ENROLL" -eq 1 ]]; then
        ...
        # Install xnpanel CLI BEFORE the enrollment callback so the
        # version file (/etc/xnpanel/version) lands on disk regardless of
        # what happens in enroll_complete. The panel's per-node version
        # probe falls back to that file when its update-cache hasn't been
        # populated yet (timer fires 2 min after boot), so writing it
        # late means a freshly-enrolled node shows up as "— — —" in the
        # admin upgrade-all table for the first couple of minutes.
        install_xnpanel_cli
        enroll_complete
        print_enroll_summary
        return
